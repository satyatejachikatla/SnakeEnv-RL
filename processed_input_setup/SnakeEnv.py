...
class Snake:
	...
	def get_state(self):
		...
		field_size = np.array(self.play_field.shape)
		head_location = self.snake_body_points[-1]
		snack_location = np.array(self.snack_cell)

		distance_to_walls = np.concatenate([field_size - head_location, head_location ])
		distance_to_snack = snack_location - head_location

		return np.array(np.concatenate([head_location,snack_location,distance_to_walls,distance_to_snack]),dtype=np.float32)
